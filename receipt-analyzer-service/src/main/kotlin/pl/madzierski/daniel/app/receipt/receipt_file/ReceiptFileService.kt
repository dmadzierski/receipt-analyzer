...
class ReceiptFileService(private val receiptFileRepository: ReceiptFileRepository) {
    ...
    fun createAndSave(receiptRevision: ReceiptRevisionEntity, file: MultipartFile): ReceiptFileEntity {

        val receiptFile = ReceiptFileEntity(receiptRevision, null)
        ...
        return receiptFileRepository.save(savedReceiptFileWithPath)
    }
    ...
    fun getFileReceipt(receiptFileId: String): FileSystemResource {
        val file = File(
            receiptFileRepository.findById(receiptFileId)
                .orElseThrow { AppRuntimeException(AppRuntimeExceptionMessages.FILE_RECEIPT_NOT_FOUND) }
                .path ?: throw AppRuntimeException(AppRuntimeExceptionMessages.FILE_RECEIPT_PATH_NOT_FOUND)
        )

        if (!file.exists())
            throw AppRuntimeException(AppRuntimeExceptionMessages.FILE_RECEIPT_FILE_NOT_FOUND)
